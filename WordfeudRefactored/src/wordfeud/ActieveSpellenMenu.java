...
public class ActieveSpellenMenu extends JPanel implements ActionListener{
	...
	public void showSpellen(){
		...
		rs = connect.voerSelectQueryUit("select * from Spel where (Account_naam_tegenstander ='"+Account.getAccountNaam()+"' OR Account_naam_uitdager='"+Account.getAccountNaam()+"') AND Toestand_type = 'Playing'");
		try {
			while(rs.next())
			{
				...
				//Wie is de tegenstander in het spel? Degene die je niet zelf bent ofcourse
				String p1 = rs.getString("Account_naam_uitdager");
				String p2 = rs.getString("Account_naam_tegenstander");
				String tegenstander = (p1.equals(Account.getAccountNaam())) ? p2 : p1;
				...
				JLabel uitdagingTxt = new JLabel(tegenstander + " [" + comp_naam + "]");
